...
@requires_db
@explore_docs.document(url_prefix + "/tradable", 'GET', 'Returns all contracts that are tradable.')
def get_all_tradable():
    contracts = GetAllTradableContracts().execute_n_fetchall({}, close_connection=True, load_out=True)
    if contracts is not None:
        log_kv(LOG_DEBUG, {'debug': 'succesfully got all contracts'})
        return success_response({'contracts': contracts})
    else:
